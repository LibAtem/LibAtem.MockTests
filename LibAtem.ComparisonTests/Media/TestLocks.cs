        ...
        public void TestLock()
        {
            using (var helper = new AtemComparisonHelper(_client, _output))
            {
                ...
                foreach (LockStateChangedCommand lck in lockCommands)
                {
                    ...
                    // lock via lib
                    helper.SendAndWaitForMatching(new CommandQueueKey(new LockObtainedCommand() { Index = lck.Index }), new LockStateSetCommand() { Index = lck.Index, Locked = true });
                    helper.Sleep();
                    ...
                    // unlock via lib
                    helper.SendAndWaitForMatching(new CommandQueueKey(new LockStateChangedCommand() { Index = lck.Index }), new LockStateSetCommand() { Index = lck.Index, Locked = false });
